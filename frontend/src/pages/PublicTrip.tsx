...
const PublicTrip = () => {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center"><div className="w-12 h-12 border-4 border-purple-600 border-t-transparent rounded-full animate-spin"></div></div>;
        ...
        <div className="absolute bottom-0 left-0 right-0 p-12 max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
             <div className="flex items-center gap-3">
                <span className="bg-purple-600 text-white px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest">Shared Itinerary</span>
             ...
             <div className="flex flex-wrap items-center gap-6 text-white/90">
                <div className="flex items-center gap-2 font-bold"><Calendar className="w-5 h-5 text-purple-400" /> {new Date(trip.startDate).toLocaleDateString()} - {new Date(trip.endDate).toLocaleDateString()}</div>
                <div className="flex items-center gap-2 font-bold"><MapPin className="w-5 h-5 text-purple-400" /> {trip.stops?.length} Destinations</div>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
               <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-gray-100">
                  <h3 className="text-base font-black text-gray-900 mb-8 tracking-tight">Trip Journey</h3>
                  <div className="space-y-12">
                     {trip.stops?.map((stop: any, idx: number) => (
                       <div key={stop.id} className="relative pl-12 border-l-2 border-dashed border-gray-100 last:border-0 pb-12 last:pb-0">
                          <div className="absolute -left-[11px] top-0 w-5 h-5 rounded-full bg-purple-600 shadow-[0_0_15px_rgba(59,130,246,0.5)] flex items-center justify-center text-white text-[8px] font-black">
                             {idx + 1}
                          </div>
                          <div className="flex justify-between items-start mb-6">
                             <div>
                                <h4 className="text-base font-black text-gray-900 tracking-tight">{stop.cityName}</h4>
                                <p className="text-purple-600 font-black text-xs uppercase tracking-widest mt-1">{stop.country}</p>
                             </div>
                          </div>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                             {stop.activities?.map((act: any) => (
                               <div key={act.id} className="bg-gray-50 p-6 rounded-[2rem] border border-gray-100 hover:border-purple-200 transition-all group">
                                  <div className="flex items-center gap-4 mb-3">
                                     <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center shadow-sm text-purple-600 group-hover:scale-110 transition-transform">
            ...
            <div className="space-y-8">
               <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-gray-100 text-center">
                  ...
                  <h4 className="text-base font-black text-gray-900 mb-2">Like this trip?</h4>
                  ...
                  <button 
                    onClick={handleClone}
                    className="w-full bg-purple-600 text-white py-5 rounded-2xl font-black text-base hover:bg-purple-700 shadow-xl shadow-purple-100 transition-all active:scale-95 flex items-center justify-center gap-3"
                  >
               ...
               <div className="bg-gray-900 p-8 rounded-[3rem] shadow-xl text-white">
                  <h4 className="text-base font-black mb-6 flex items-center gap-2"><Share2 className="w-5 h-5 text-purple-400" /> Share Journey</h4>
